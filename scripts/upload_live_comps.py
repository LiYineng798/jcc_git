...
def parse_args(argv=None):
    ...
    parser.add_argument('--season-id', default='', help='Optional live comps season id appended as ?season=...')
    return parser.parse_args(argv)


def upload_payload(file_path, url, token, timeout=180, season_id='', opener=urllib.request.urlopen):
    with open(file_path, 'r', encoding='utf-8') as file:
        payload = json.load(file)
    body = json.dumps(payload, ensure_ascii=False).encode('utf-8')
    if season_id:
        separator = '&' if '?' in url else '?'
        url = f'{url}{separator}season={urllib.parse.quote(str(season_id))}'
...
def main(argv=None, opener=urllib.request.urlopen):
    ...
    try:
        print(upload_payload(args.file, args.url, args.token, timeout=args.timeout, season_id=args.season_id, opener=opener))
        return 0
