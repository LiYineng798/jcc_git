...
def test_browser_style_add_lineup_after_register(client):
    ...
    response = client.post('/api/lineups', json={'name': '测试阵容', 'code': 'TEST-CODE'}, headers={'X-CSRF-Token': me['csrf_token']})
...
def test_browser_style_add_lineup_after_admin_login(client):
    ...
    me = client.get('/api/me').get_json()
    response = client.post('/api/lineups', json={'name': '管理员阵容', 'code': 'ADMIN-CODE'}, headers={'X-CSRF-Token': me['csrf_token']})
    assert response.status_code == 201
